  ...
  try {
    ...
    for (const variant of variants) {
      ...
      await sql`
        ...
        VALUES (${variantId}, ${id}, ${variant.label}, ${variant.price}, ${stockQty}, ${variant.stockLevel ?? 'none'}, TRUE)
      `;

      // Handle variant image
      const variantImageFile = formData.get(`variantImage_${variant.id}`) as File | null;
      if (variantImageFile) {
        const blob = await put(`products/${id}/variants/${variantId}/${variantImageFile.name}`, variantImageFile, {
          access: "public",
        });
        await sql`
    ...
    if (newImages.length > 0 || productImages.length > 0) {
      const existingImages: any[] = await sql`SELECT id, image_url FROM images WHERE product_id = ${id}`;
      const existingImageIds = existingImages.map(img => img.id);
      const productImageIdsToKeep = productImages.filter((img: any) => img.id).map((img: any) => img.id);
      const imagesToDelete = existingImages.filter(img => !productImageIdsToKeep.includes(img.id));
      ...
      await sql`DELETE FROM images WHERE product_id = ${id} AND id NOT IN (${productImageIdsToKeep.length > 0 ? productImageIdsToKeep.join(",") : "NULL"})`;
...
export async function DELETE(
  ...
  try {
    ...
    const images = await sql`SELECT image_url FROM images WHERE product_id = ${id}`;
